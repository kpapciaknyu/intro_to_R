...
```{r}
patients <- read.csv("~/Downloads/patients.csv") #couldnt pull from permalink for some reason so I downloaded file
...
```{r}
library(dplyr)
observations <- read.csv("~/Downloads/observations.csv")
...
```{r}
# 1. First, examine the data types
str(merged_subset$`Body Weight`)
str(merged_subset$`Body Height`)

# 2. Convert columns to numeric, handling any non-numeric values
merged_subset$`Body Weight` <- as.numeric(as.character(merged_subset$`Body Weight`))
merged_subset$`Body Height` <- as.numeric(as.character(merged_subset$`Body Height`))

# 3. Check for NA values created during conversion
sum(is.na(merged_subset$`Body Weight`))
sum(is.na(merged_subset$`Body Height`))

# 4. Convert height to meters (assuming it's in cm)
merged_subset$`Body Height in m` <- merged_subset$`Body Height` / 100

# 5. Calculate BMI only for rows where both weight and height are available
merged_subset$BMI <- NA  # Initialize with NA
valid_rows <- !is.na(merged_subset$`Body Weight`) & !is.na(merged_subset$`Body Height in m`)
merged_subset$BMI[valid_rows] <- merged_subset$`Body Weight`[valid_rows] / 
                                 (merged_subset$`Body Height in m`[valid_rows]^2)

# 6. Check the results
head(merged_subset[, c("Body Weight", "Body Height", "Body Height in m", "BMI")])
```
